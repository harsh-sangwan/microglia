import sys
from read_img import read_nifti
import pandas as pd
...
def get_blobs_dist_dict(vol, patch_offset=None):
    #nn = get_nearest_neighbors(pred)
    blob_dist = {}
    for b1 in vol:
        blob_dist[b1['id']] = {}
        for b2 in vol:
            #find distance between centroids as some blobs may have more points than rest
            #c1 = np.round(np.mean(b1['points'], axis=0))
            #c2 = np.round(np.mean(b2['points'], axis=0))

            #correct for patch_offset
            if patch_offset:
                b1['centroid'] += patch_offset
                b2['centroid'] += patch_offset
            blob_dist[b1['id']][b2['id']] = point_dist(b1['centroid'], b2['centroid'])


    blob_dist_df = pd.DataFrame(blob_dist)
    blob_dist_df = blob_dist_df.replace(0.0, np.nan)
    blob_dist_df['nn'] = blob_dist_df.min(axis=0, skipna=True)
    return blob_dist_df['nn'].to_dict()

def get_blobs_dist_vol(blob_vol, patch_vol, patch_offset=None):
    patch_vol_copy = patch_vol.copy()
    blobs = []

    if len(blob_vol) < 2:
        return None, None

    nearest_dist_dict = get_blobs_dist_dict(blob_vol)

    for blob in blob_vol:
        blob_dict = {}
        blob_dict['id'] = blob['id']
        blob_dict['centroid'] = blob['centroid']

        if patch_offset is not None:
            blob_dict['patch_offset'] = patch_offset
            blob_dict['offset_centroid'] = patch_offset + blob['centroid']


        blob_dict['patch_nearest_dist'] = np.round(nearest_dist_dict[blob_dict['id']], 4)
        blob_dict['points'] = blob['points']
        #for pt in blob['points']:
        #    patch_vol_copy[pt[0], pt[1], pt[2]] = blob_dict['nearest_dist']
        blobs.append(blob_dict)

    return pd.DataFrame(blobs).set_index('id'), patch_vol_copy
...
def get_patch_overlap(pred_patch, target_patch):
    ...
    if pred is None and target is None:
        return 0, 0, 0, None
    #blobs_centroids = save_centroids_csv(pred)
    #blobs_dist_csv = get_blobs_distances(pred)
    ...
    return tp, fp, fn, pred#, blobs_centroids
...
def get_blobs(volume):
    ...
    for label in labels:
        ...
        blob['centroid'] = np.round(np.mean(blob['points'], axis=0))
