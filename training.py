...
from utils import minmax_normalize, create_overlay, get_3D_slices, get_blob_based_perf, get_spliced_arrays, train_preprocessing, validation_preprocessing
from read_img import read_nifti, write_nifti
from models import custom_unet, custom_unet2
...
cx3_orig_patches_dir = 'cx3_orig_patches/'
cx3_empty_patches_dir = 'cx3_empty_patches/'
iba1_patches_dir = 'iba1_orig/'

#raw_files_path = 'training_patches/raw/'
#gt_files_path = 'training_patches/gt/'
...
if __name__ == "__main__":
    ...
    cx3_orig_patch_nums = [x.split('.')[0].split('_')[1] for x in os.listdir(cx3_orig_patches_dir + 'raw/')]
    cx3_empty_patch_nums = [x.split('.')[0].split('_')[1] for x in os.listdir(cx3_empty_patches_dir + 'raw/')]
    iba1_patch_nums = [x.split('.')[0].split('_')[1] for x in os.listdir(iba1_patches_dir + 'raw/')]

    patches = cx3_orig_patch_nums + cx3_empty_patch_nums + iba1_patch_nums

    for patch_num in patches:
        patch_data[patch_num] = {}
        if patch_num in cx3_orig_patch_nums:
            patch_data[patch_num]['raw']    = read_nifti(cx3_orig_patches_dir + 'raw/patchvolume_' + str(patch_num) + '.nii.gz')
            patch_data[patch_num]['gt']     = read_nifti(cx3_orig_patches_dir + 'gt/patchvolume_' + str(patch_num) + '.nii.gz')

        elif patch_num in cx3_empty_patch_nums:
            patch_data[patch_num]['raw'] = read_nifti(
                cx3_empty_patches_dir + 'raw/patchvolume_' + str(patch_num) + '.nii.gz')
            patch_data[patch_num]['gt'] = read_nifti(
                cx3_empty_patches_dir + 'gt/patchvolume_' + str(patch_num) + '.nii.gz')

        elif patch_num in iba1_patch_nums:
            patch_data[patch_num]['raw'] = read_nifti(
                iba1_patches_dir + 'raw/patchvolume_' + str(patch_num) + '.nii')
            patch_data[patch_num]['gt'] = read_nifti(
                iba1_patches_dir + 'gt/patchvolume_' + str(patch_num) + '.nii.gz')
    ...
    #test_patch_seq = ['2147', '9425', '3897']#random.sample(iba1_patch_nums, 3) #
    test_patch_seq = random.sample(cx3_empty_patch_nums, 3) + random.sample(cx3_orig_patch_nums, 3) + random.sample(iba1_patch_nums, 3) #
    ...
    for i in range(5):
        ...
        training_patches = [p for p in patches if p not in test_patch_seq] #p in iba1_patch_nums and

        #+ random.sample([p for p in training_patches if p in iba1_patch_nums], 3)
        val_patch_seq = random.sample([p for p in training_patches if p in iba1_patch_nums], 4) +\
                        random.sample([p for p in training_patches if p in cx3_empty_patch_nums], 2) + \
                        random.sample([p for p in training_patches if p in cx3_orig_patch_nums], 4) #+ \
                         # +
        train_patch_seq = [p for p in training_patches if p not in val_patch_seq]

        X_train = [minmax_normalize(patch_data[pn]['raw'], min_val=0, max_val=2**16) for pn in train_patch_seq]
        y_train = [patch_data[pn]['gt'] for pn in train_patch_seq]

        X_val = [minmax_normalize(patch_data[pn]['raw'], min_val=0, max_val=2**16) for pn in val_patch_seq]
        y_val = [patch_data[pn]['gt'] for pn in val_patch_seq]

        X_test = [minmax_normalize(patch_data[pn]['raw'], min_val=0, max_val=2**16) for pn in test_patch_seq]
        ...
        def get_dataset(X, y, shuffle=False, set='train'):
            dataset = tf.data.Dataset.from_tensor_slices((X, y))

            if set == 'train':
                #dataset = dataset.shuffle(buffer_size=1).map(train_preprocessing).batch(batch_size).prefetch(1)
                dataset = dataset.shuffle(buffer_size=1).batch(batch_size).prefetch(1)
            elif set == 'val':
                #dataset = dataset.map(validation_preprocessing).batch(batch_size).prefetch(1)
                dataset = dataset.batch(batch_size).prefetch(1)
            else:
                dataset = dataset.batch(batch_size).prefetch(1)
            return dataset

        train_dataset = get_dataset(X_train, y_train, shuffle=True, set='train')
        val_dataset = get_dataset(X_val, y_val, set='val')
        ...
        for pi in range(len(train_patch_seq)):
            fold_performance = update_fold_perf(fold_performance, train_patch_seq[pi], y_train[pi], train_preds[pi],
                                                'train')

        for pi in range(len(val_patch_seq)):
            fold_performance = update_fold_perf(fold_performance, val_patch_seq[pi], y_val[pi], val_preds[pi],
                                                'val')

        for pi in range(len(test_patch_seq)):
            fold_performance = update_fold_perf(fold_performance, test_patch_seq[pi], y_test[pi], test_preds[pi],
                                                'test')
    ...
    #chunk_perf[splice_by] = overall_folds_metrics[best_fold]

    # save loss functions
    #get_model_loss_plots(best_history, plot_fname='losses_splice_by_' + str(splice_by) + '.png')
    get_model_loss_plots(best_history, plot_fname='losses.png')
    ...
    for i in range(len(y_test)):
        ...
        pred_patch = np.reshape(test_preds[i], (orig_size, orig_size, orig_size))
        ...
        if predicted_blobs is not None:
            blobs_dist_df, blobs_dist_vol = get_blobs_dist_vol(predicted_blobs, pred_patch)

            if blobs_dist_df is not None:
                blobs_dist_df.to_csv(iba1_patches_dir + 'blobs_dist_csv/patchvolume_'+str(patch_num)+'_dist.csv')
            if blobs_dist_vol is not None:
                write_nifti(iba1_patches_dir + 'blobs_dist_somata/patchvolume_' + str(patch_num) + '.nii.gz', blobs_dist_vol)
    ...
    print("Overall Test Set Blob Dice : ", test_set_tp / (0.00001 + test_set_tp + 0.5 * (test_set_fp + test_set_fn)))
    ...
    for k in range(len(test_patch_seq)):
        pn = test_patch_seq[k]
        output_volume = np.reshape(test_preds[k], (orig_size, orig_size, orig_size))
        write_nifti(iba1_patches_dir+'output/patchvolume_'+str(pn)+'.nii.gz', output_volume)

        #create overlay of predictions with gt
        overlay_volume = create_overlay(y_test[k], np.reshape(test_preds[k], (orig_size, orig_size, orig_size)))
        write_nifti(iba1_patches_dir + 'overlays/patchvolume_'+str(pn)+'.nii.gz', overlay_volume)
